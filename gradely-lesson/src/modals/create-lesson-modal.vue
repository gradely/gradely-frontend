  ...
  <div>
    <modal-cover @closeModal="$emit('closeTriggered')">
      <template slot="modal-cover-header">
        <div class="gfont-15 font-weight-700 color-text text-uppercase p-3">Create Lesson</div>
      </template>
      ...
      <template slot="modal-cover-body">
        <div class="modal-cover-body px-3 py-2 position-relative">
          ...
          <div class="lesson-details-wrapper mgb-20">
            <div class="icon-wrapper" :class="$doc.getDocBgcolor(getFileExtension) + '-bg'">
              ...
              <img v-lazy="getImageSrc" alt class="file-thumbnail" v-if="isMedia" />
            ...
            <div class="lesson-detail">
              ...
              <div class="lesson-meta color-grey-dark">
                <span class="mgr-5">
                  {{
      ...
      <template slot="modal-cover-footer">
        <div class="modal-footer mgt-15">
          <div class="icon-container pointer"  @click="$emit('closeTriggered')">
            <span class="icon icon-trash gfont-20 color-ash"></span>
            <span class="line"></span>
          </div>
          <button class="btn rounded-15 btn-accent" @click="uploadLessonFile" ref="post">POST</button>
        </div>
      </template>
    </modal-cover>
  </div>
...
export default {
  ...
  computed: {
    ...
    getUploadLessonPayload() {
      return {
        ...
        attachments: [
          {
            title: this.getFileName,
            file_name: this.lesson_file_url,
            file_type: this.getFileType,
            extension: this.getFileExtension,
            file_size: this.uploaded_file?.size / 1000 + 'kb',
          },
        ],
      };
    },
    ...
    isVideo() {
      let type = this.uploaded_file?.type?.split('/')[0];
      return type === 'video';
    },
    ...
    getFileExtension() {
      let name_splits = this.uploaded_file?.name?.split('.');
      let ext_index = name_splits.length - 1;
      return name_splits[ext_index];
    },
    ...
    getFileName() {
      let names = this.uploaded_file?.name?.split('.');
      names.pop();
      let name = names.join('');
    ...
    getImageSrc() {
      let url = '';
      if (!this.uploaded_file || !this.isMedia) return url;
      if (this.isImage) return URL.createObjectURL(this.uploaded_file);
      if (this.isVideo) return this.video_src;
      return url;
    },
  ...
  methods: {
    ...
    ...lesson.mapActions(['uploadLesson']),
    ...
    uploadLessonMaterial() {
      this.uploadLesson(this.getUploadLessonPayload)
        .then((response) => {
          this.handleClick('post', 'post', false);
          if (response.code === 200) {
            this.pushAlert('lesson posted', 'success');
