<template>
  ...
  >
      ...
      <div class="left">
        ...
        <div>
          ...
          <div class="meta-text color-grey-dark mgt-4">
            Find notes shared in your class here.
          </div>
        </div>
      </div>
    ...
    <template v-if="documents.length">
      <document-collection :docs="getGroupedDocuments" />
    </template>
    ...
    <template v-else>
      <default-skeleton-loader
        ...
        :empty="{
          title: 'No Documents!',
          message: 'You currently do not have any library documents available!',
        }"
        :cta="{
          has_cta: false,
          cta_text: '',
    ...
    <template>
      <div class="gfont-13 text-center mgt-40 mgb-10">
        Showing {{ documents.length }} of {{ getTotalItemCount }}
      </div>
...
import documentCollection from "@/modules/base/components/feed-comps/post-library-comps/document-collection";
...
export default {
  ...
  computed: {
    ...
    getGroupedDocuments() {
      return this.documents.reduce((groupDocs, doc) => {
        groupDocs[doc.upload_term_week]
          ? groupDocs[doc.upload_term_week].push(doc)
          : (groupDocs[doc.upload_term_week] = [doc]);
        return groupDocs;
      }, {});
    },
    ...
    progressSyle() {
      ...
      let progress = (this.documents?.length / this.getTotalItemCount) * 100;
      return `width:${progress}%`;
    },

    disabled() {
      return (
        this.documents?.length >= this.getTotalItemCount ||
        !this.can_search ||
        this.loading
      );
    },
  ...
  watch: {
    ...
    getPayloadFilter() {
      ...
      this.can_search ? this.fetchLibraryDocs() : this.deferredSearch();
    },
  },
  ...
  methods: {
    ...mapActions({
      getLibraryDocuments: "dbFeeds/getLibraryDocuments",
    ...
    loadMoreDocs() {
      ...
      this.getLibraryDocuments(this.getPayload)
        .then((response) => {
          this.loading = false;
          ...
          if (response.code === 200) {
            ...
            this.pushAlert(
              "Can't load more document at the moment.",
              "warning"
            );
          }
        })
        .catch(() => {
          this.loading = false;
          this.pushAlert("Can't load more document at the moment.", "error");
        });
    },
