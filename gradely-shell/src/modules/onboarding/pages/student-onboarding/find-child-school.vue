  ...
  <div class="connect-school">
    <onboarding-nav-layout>
      <template slot="content">
        <div>
          <!-- IMAGE TOP  -->
          <div class="onboarding-image-top">
            <img
              v-lazy="
                `${getAssetBaseURL(
                  'cloud',
          ...
          <div class="onboarding-form-wrapper mgt-50 mx-auto">
            <div class="form-group compact-row w-100">
              ...
              >
                <div v-if="!verifying" @click="verifyCode(true)">
                  {{ verify_feedback }}
                </div>
            ...
            <div class="info-text color-ash text-center">
              ...
              <router-link :to="{ name: 'StudentInviteSchool' }">
      ...
      this.verifyClassCode(this.class_code)
        .then((response) => {
          ...
          if (response.code === 200) {
            ...
            this.school_name =
              response?.data?.school?.name || response?.data?.class_code;
            this.verified_code = response?.data?.class_code;
          } else {
            if (status)
        ...
        .catch(() => {
          ...
          this.school_name = "";
          this.verifying = false;
          if (status) this.pushAlert("Error verifying code", "error");
        });
    },
      ...
      this.studentJoinClass(payload)
        .then(async (response) => {
          ...
          if (response.code === 200) {
            ...
            setTimeout(() => {
              location.href = this.$route.query.child_id
                ? `/feed/${this.$route.query.child_id}`
                : "/onboarding/select-avatar";
            }, 1000);
          } else {
            this.pushAlert(
              response?.message || "Unable to join class",
              "warning"
            );
